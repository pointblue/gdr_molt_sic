...
# }
  ...
  summ_grid <- rasterize(data_summ[,c("x_bin","y_bin")],y=r,
                         field=data_summ$n_locs)
  
  
  

  
  # # Define extent and create a raster
  # e <- ext((min(data_summ$x_bin) - grid_size*2), 
  #            (max(data_summ$x_bin) + grid_size*2), 
  #            (min(data_summ$y_bin) - grid_size*2), 
  #            (max(data_summ$y_bin) + grid_size*2))
  # 
  # r <- rast(e, resolution = grid_size, crs = proj_ant)
  # 
  # # Convert data frame to sf object
  # data_summ_sf <- st_as_sf(data_summ, coords = c("x_bin", "y_bin"), crs = proj_ant)
  # 
  # # Convert the sf object to a SpatVector
  # v <- vect(data_summ_sf)
  # 
  # # Rasterize the SpatVector
  # summ_grid <- terra::rasterize(v, r, field = "n_locs")
  ...
  if(scaled_rast){
    ...
    # summ_SPixDF <- as.data.frame(summ_grid_scl)
  }else{
    summ_SPixDF<-as.data.frame(as(summ_grid,"SpatialPixelsDataFrame"))
  }
  # get contour lines
  # mat <- matrix(values(summ_grid_scl), nrow = ncol(summ_grid_scl), ncol = nrow(summ_grid_scl))
  # cont_50 <-
  #   contourLines(z = mat, x = xFromCol(summ_grid_scl, 1:ncol(summ_grid_scl)), y = yFromRow(r, 1:nrow(summ_grid_scl)), levels=c(50))
  # 
  # 
...
# convert spatial lines to polygons  
  ...
  cont_50_poly_sp <-SpatialPolygons(list(cont_50_poly_ls))

  # clip to coastline
  cont_50_poly_clip <- cont_50_poly_sp - ant_clip 
  # cont_50_poly_clip <- gDifference(cont_50_poly_sp, ant_clip)
  plot(cont_50_poly_clip)
  ...
  # then convert to spatial Polygons
  cont_95_poly_sp <-SpatialPolygons(list(cont_95_poly_ls))
  # clip to coastline
